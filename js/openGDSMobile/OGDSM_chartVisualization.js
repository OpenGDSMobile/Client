...
(function (OGDSM) {
    ...
    */
    OGDSM.chartVisualization = function (jsonData, options) {
        ...
        if (typeof (this.defaults.rootKey) === 'undefined' ||
                typeof (this.defaults.labelKey) === 'undefined' ||
                typeof (this.defaults.valueKey) === 'undefined') {
            console.error('Please input option values : rootKey, label, value');
            return null;
        }
        this.jsonData = jsonData;
        this.data = jsonData[this.defaults.rootKey];
        var d = null;
        this.defaults.max = this.defaults.min = this.data[0][this.defaults.valueKey];
        for (d in this.data) {
            if (this.data.hasOwnProperty(d)) {
                this.defaults.max = Math.max(this.data[d][this.defaults.valueKey], this.defaults.max);
                this.defaults.min = Math.min(this.data[d][this.defaults.valueKey], this.defaults.min);
            }
        }
        this.defaults = OGDSM.applyOptions(this.defaults, options);
        console.log(this.defaults.min + ' ' + this.defaults.max);
    };
...
OGDSM.chartVisualization.prototype.vBarChart = function (rootDiv, barChartOptions) {
    ...
    var barDiv = d3.select('#' + rootDiv).append('svg').attr('id', rootDiv + 'Bar')
    ...
    var bar = barDiv.selectAll('g').data(data).enter().append('g').attr('transform', function (d, i) {
    ...
    bar.append('rect')
        ...
        .attr('x', function (d, i) {
            return labels.rangeBand() + (margin.left / 4);
        })
        .attr('height', function (d) {
            return barHeight - d[options.valueKey];
        })
