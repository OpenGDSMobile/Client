...
openGDSM.publicOpenData.env = {
		...
		$.ajax({
			type:'POST',
			url:serverURL+serverFolder+'PublicDataPortal.do',
		...
		mapVisual: function(data){
			...
			envMap.setStyle(function(f,r){
				var text;
				if(openGDSM.publicOpenData.env.mapLayer =='Seoul_si' || openGDSM.publicOpenData.env.mapLayer=='Seoul_dong') text =r < 5000 ? f.get('SIG_KOR_NM') : '';
